...
To interact with Tetra Data Platform task scripts you will need to use the context object. The context object provides functions for you to read files, write files, get pipeline configuration information, and more. This topic contains:
...
📘 NOTE:  If a gzipped file is larger than 100MB, the file will be uploaded as multi-part upload.
...
🚧 WARNING:  Notice you can only define file_suffix, you cannot define the complete file name of the output IDS file. The file name is codified and follows a certain pattern, which includes ids_version. This means if you upgraded your raw-to-ids protocol and the new protocol is using a new IDS version, the new IDS JSON produced will be a separate file not a new version of the old file.
...
📘 NOTE: This function is still in the early stage and requires Tetra DataHub and Agent support. It's not an out-of-shelf function that you can use on any instrument. Please contact the TetraScience team for more information.
...
The secret parameter must be a ‘dict’ type (an associative array - key:value). If it is not, or if the value is not a secret, the parameter that you entered will be returned.
